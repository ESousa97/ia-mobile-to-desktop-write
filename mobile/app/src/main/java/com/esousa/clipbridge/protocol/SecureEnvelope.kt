...
object SecureEnvelope {
    ...
    fun encrypt(plaintext: Envelope, cipher: SessionCipher, json: Json): Envelope {
        ...
        val package = cipher.encrypt(payloadJson.toByteArray(), associatedData(plaintext))
        return plaintext.copy(
            payload = json.encodeToJsonElement(EncryptedPayload(Base64.getEncoder().encodeToString(package))).jsonObject,
        )
    }
