...
|---|---|---|
| **S**poofing | Dispositivo malicioso finge ser o par pareado | Chaves por dispositivo (TOFU), verificação de fingerprint, token de pareamento de uso único |
...
```
   ...
   LAN (semiconfiável)  ──── tráfego cifrado E2E ────
        │
   Dispositivos pareados (confiáveis após verificação de fingerprint)
```
